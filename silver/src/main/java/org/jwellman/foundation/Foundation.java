...
public class Foundation extends Platinum {
    ...
    // ========================================================================
    // VIEW PROVIDER - CARD-BASED VIEW MANAGEMENT (Silver Tier)
    // ========================================================================

    /**
     * Adds a new card (view) to the managed container.
     * <p>
     * This method allows applications to dynamically add views for:
     * <ul>
     * <li>Multi-step wizards</li>
     * <li>Mode switching (viewer mode → editor mode)</li>
     * <li>Custom splash screens</li>
     * </ul>
     * <p>
     * The framework automatically creates a "main" card for the primary
     * application content. Additional cards can be added as needed.
     * <p>
     * <strong>Requires Silver tier or above.</strong>
     *
     * @param cardName Unique identifier for this card
     * @param content The component to display when this card is shown
     * @throws IllegalStateException if called before Foundation.init()
     * @throws IllegalArgumentException if cardName is null or empty
     * @since Silver Tier
     */
    public static void addCard(String cardName, java.awt.Component content) {
        if (instance == null || instance.masterContext == null) {
            throw new IllegalStateException("Foundation not initialized - call Foundation.init() first");
        }

        if (!(instance.masterContext instanceof org.jwellman.foundation.uContext)) {
            return; // Not a concrete uContext, cannot access view provider
        }

        org.jwellman.foundation.uContext context = (org.jwellman.foundation.uContext) instance.masterContext;
        org.jwellman.foundation.interfaces.uiViewProvider viewProvider = context.getViewProvider();

        if (viewProvider == null) {
            // Not using view provider (Stone/Bronze tier or view provider not enabled)
            return;
        }

        viewProvider.addCard(cardName, content);
    }

    /**
     * Switches the visible card to the specified name.
     * <p>
     * This method allows applications to control which view is currently visible.
     * Common use cases:
     * <ul>
     * <li>Dismissing splash: showCard("main")</li>
     * <li>Wizard navigation: showCard("step2")</li>
     * <li>Mode switching: showCard("editor")</li>
     * </ul>
     * <p>
     * <strong>Requires Silver tier or above.</strong>
     *
    ...
    public static void showCard(String cardName) {
        ...
        viewProvider.showCard(cardName);
    }
